...
This paper documents the transparent screening framework used in the \emph{ImpactLLM Observatory}. The observatory now covers 71 models, and this manuscript explains how the interface extracts scenarios from natural-language descriptions, propagates them through bounded multi-factor screening proxies, and publishes the resulting comparative tables and figures. The goal is to keep those proxies auditable while keeping the application workflow fast and interpretable.
...
\end{equation}
where $P_t$ is the retained parameter count, $Tok_t$ is a training-token prior, $F^{reg}$ is a training-regime prior, $F^{arch-tr}$ captures architecture and multimodality assumptions, and $F^{hw}$ is a hardware-class proxy. When anchor sources explicitly report training-token counts, those values are retained for the anchor rather than replaced by a generic rule. The value of 20 training tokens per retained parameter is currently used as a project screening prior only when no model-specific public estimate is available. It should be read as a pragmatic default assumption, not as a direct measurement. This default is broadly consistent with the order of magnitude illustrated by Chinchilla, whose compute-optimal example combines 70B parameters with 1.4T training tokens, i.e. about 20 tokens per parameter \cite{hoffmann2022chinchilla}. The current release also defaults market models to a foundation-pretraining regime unless a narrower public indication exists.
...
\hline
GPT-5.5, one standardized request & 3.0833 Wh & 1.1871 gCO$_2$e & Latest frontier proprietary model under a US provider-country proxy \\
\hline
Claude Mythos Preview, one standardized request & 3.8014 Wh & 1.4635 gCO$_2$e & Restricted frontier preview illustrating the current upper end of the screening catalog \\
\hline
...
\hline
Claude Mythos Preview & 3.8014 & 1.4635 & 139.48 & Frontier preview model with the highest current screening values in the catalog \\
\hline
GPT-5.5 & 3.0833 & 1.1871 & 74.50 & Latest frontier proprietary model with very high screening values for both inference and training \\
\hline
Gemini 3.1 Pro & 0.4282 & 0.1649 & 1.07 & Upper-mid-range hosted multimodal model with materially lower training magnitude than the frontier tier \\
\hline
DeepSeek V4 Pro & 0.1249 & 0.0675 & 41.46 & Recent open/API MoE model whose retained total-parameter count lifts training estimates sharply \\
\hline
Mistral Large 3 & 0.0925 & 0.0037 & 8.49 & Open-hybrid multimodal MoE model with modest French-carbon contextualization at inference time \\
\hline
Llama 4 Scout & 0.0402 & 0.0155 & 4.26 & Open multimodal MoE model anchored by published training-token and emissions disclosures \\
\hline
Qwen3 32B & 0.0508 & 0.0274 & 0.02 & Dense open model with low-to-mid screening values \\
\hline
Ministral 3B & 0.0056 & 0.0002 & 0.0002 & Low-end model illustrating the lower bound of the current catalog \\
\hline
\end{tabularx}
